...
- PostgreSQL migrations and the persistence/concurrent-write integration test passed on GitHub CI. Locally the database test is skipped because this environment provides neither a PostgreSQL daemon nor Docker.
...
- GitHub CI passed migrations, lint, type checking, all 30 tests (including PostgreSQL persistence), and the production build on Node 22. Both default and Webpack local builds stopped before compilation with `ENOENT: uv_resident_set_memory`, an environment/runtime memory-inspection failure; no check was hidden or disabled.
